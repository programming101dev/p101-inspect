...
{
    const struct p101_tool_analysis_finding *const *left_finding;
    const struct p101_tool_analysis_finding *const *right_finding;
    int                                             p101_single_result_;

    left_finding        = (const struct p101_tool_analysis_finding *const *)left;
    right_finding       = (const struct p101_tool_analysis_finding *const *)right;
    p101_single_result_ = strcmp((*left_finding)->diagnostic_id, (*right_finding)->diagnostic_id);
    if(p101_single_result_ == 0)
    {
        p101_single_result_ = strcmp((*left_finding)->file_name, (*right_finding)->file_name);
    }
    if(p101_single_result_ == 0 && (*left_finding)->line_number != (*right_finding)->line_number)
    {
        p101_single_result_ = (*left_finding)->line_number < (*right_finding)->line_number ? -1 : 1;
    }
    if(p101_single_result_ == 0)
    {
        p101_single_result_ = strcmp((*left_finding)->function_name, (*right_finding)->function_name);
    }
    if(p101_single_result_ == 0)
    {
        p101_single_result_ = strcmp((*left_finding)->evidence_from, (*right_finding)->evidence_from);
    }
    if(p101_single_result_ == 0)
    {
        p101_single_result_ = strcmp((*left_finding)->evidence_to, (*right_finding)->evidence_to);
    }
    return p101_single_result_;
}
...
{
    int                                       p101_single_result_;
    const struct p101_tool_analysis_finding **findings;
    ...
    findings      = (const struct p101_tool_analysis_finding **)allocation;
    if(findings == NULL)
    {
    ...
    {
        findings[index] = p101_tool_analysis_finding_at(analysis, index);
    }
...
static int copy_file_to_stream(const char *path, FILE *destination)
{
    ...
    else
    {
        ...
        end_of_input     = feof(source);
        while(operation_status == 0 && read_error == 0 && amount > 0U)
        {
            size_t written;
            ...
            if(operation_status == 0 && end_of_input == 0)
            {
                amount       = fread(buffer, 1U, sizeof(buffer), source);
                read_error   = ferror(source);
                end_of_input = feof(source);
