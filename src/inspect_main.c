...
static int verify_artifact(struct p101_error *err, const char *directory, const char *line, struct artifact *artifacts, size_t artifact_count)
{
    ...
    operation_status = parse_artifact_line(line, &role, &role_length, &expected_bytes, &expected_records, &expected_final_newline, &expected_hash);
    if(operation_status != 0)
    {
        P101_ERROR_RAISE_ERRNO(err, EINVAL);
        p101_single_result_ = -1;
    ...
    for(size_t index = 0U; index < artifact_count; index++)
    {
        ...
        candidate_length = strlen(artifacts[index].role);
        comparison       = -1;
        if(candidate_length == role_length)
        {
            comparison = strncmp(artifacts[index].role, role, role_length);
        }
        if(candidate_length == role_length && comparison == 0)
        {
...
static int parse_artifact_line(const char *line, const char **role, size_t *role_length, size_t *bytes, size_t *records, int *final_newline, uint64_t *hash)
{
    ...
    parsed_role_length = separator == NULL ? 0U : (size_t)(separator - cursor);
    if(parsed_role_length == 0U || parsed_role_length >= ARTIFACT_ROLE_SIZE)
    {
        p101_single_result_ = -1;
        goto p101_single_exit_;
    }
    *role            = cursor;
    *role_length     = parsed_role_length;
    cursor           = separator;
...
static void locate_capture_program(const char *program)
{
    static const char fallback[] = "inspect-capture";
    static const char suffix[]   = "/inspect-capture";
    const char       *separator;

    for(size_t index = 0U; index < sizeof(fallback); index++)
    {
        capture_program[index] = fallback[index];
    }
    ...
    if(separator != NULL)
    {
        ...
        if(directory_length <= sizeof(capture_program) - sizeof(suffix))
        {
            for(size_t index = 0U; index < directory_length; index++)
            {
                capture_program[index] = program[index];
            }
            for(size_t index = 0U; index < sizeof(suffix); index++)
            {
                capture_program[directory_length + index] = suffix[index];
            }
        }
    }
}
